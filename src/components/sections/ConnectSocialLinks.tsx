...
// SVG icons for common platforms
function PlatformIcon({ name }: { name: string }) {
  ...
  if (n.includes('twitter') || n.includes('x.com') || n === 'x')
    return (
      <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current" aria-hidden>
...
function getPlatformColor(name: string): string {
  const n = name.toLowerCase()
  if (n.includes('linkedin')) return 'text-[#0A66C2]'
  if (n.includes('github')) return 'text-white'
  if (n.includes('twitter') || n === 'x' || n.includes('x.com')) return 'text-white'
  if (n.includes('instagram')) return 'text-[#E1306C]'
  if (n.includes('facebook')) return 'text-[#1877F2]'
  if (n.includes('youtube')) return 'text-[#FF0000]'
  if (n.includes('tiktok')) return 'text-white'
  if (n.includes('discord')) return 'text-[#5865F2]'
  if (n.includes('whatsapp')) return 'text-[#25D366]'
      ...
      {links.map((link) => (
        ...
        >
          <div className="flex items-center gap-3">
            <span className={`${getPlatformColor(link.platform_name)} transition-colors group-hover:opacity-90`}>
              <PlatformIcon name={link.platform_name} />
            </span>
